...
//-------------------------------------------------------------------
//Operator - (subtraksjon)

//Operator - (skiftforteikn)
...
{
  teller += nevner; //teller=teller+nevner;
  return *this;
} // operator++
...
{
  return (sammenlikn(denAndre) == 0) ? true : false;//if (sammenlikn(denAndre)==0) return true; else return false;
} // operator==
